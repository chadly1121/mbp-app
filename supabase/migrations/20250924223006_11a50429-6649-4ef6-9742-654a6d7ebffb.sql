-- First, let's create more restrictive RLS policies for qbo_connections
...
CREATE POLICY "Users can create QBO connections for their companies" 
...
CREATE POLICY "Users can update QBO connection metadata for their companies" 
ON public.qbo_connections 
FOR UPDATE 
USING (
  company_id IN (
...
CREATE POLICY "Users can delete QBO connections for their companies" 
ON public.qbo_connections 
FOR DELETE 
USING (
  company_id IN (
...
CREATE POLICY "Users can view QBO connection metadata for their companies" 
ON public.qbo_connections 
FOR SELECT 
USING (
  company_id IN (
...
-- These functions run with elevated privileges and can access tokens securely
